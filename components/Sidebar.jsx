...
const Sidebar = ({expand, setExpand})=> {
    ...
    return (
        <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max:md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden '}`} >
            <div className={`flex ${expand ? "flex-col gap-8" : "flex-col items-center gap-4" }`} >
                <div className={expand ? "flex gap-4 " : "flex flex-col gap-10"} onClick={()=>{ expand ? setExpand(false) : setExpand(true)}} >
                    <Image className= {expand ? "w-35" : "w-10"} src={expand ? assets.logo_text : assets.logo_icon} alt="" />
                    <div className="group relative flex items-center justify-center hover:bg-gray-500/20 transition-all duration-300 h-9 w-9 aspect-square rounded-lg cursor-pointer ">
                        <Image src={assets.sidebar_close_icon} alt='' className="md:hidden" />
            ...
            <div>
                <div className={`flex items-center cursor cursor-pointer group relative ${expand ? "gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer" : "h-19 w-10 mx-auto hover:bg-grey-500/30 rounded-lg"} `}>
                    ...
                    <div className={`absolute -top-60 pb-8 ${!expand && "-right-40"} opacity-0 group-hover:opacity-100 hidden group-hover:block transition`}>
                        <div className="relative w-max bg-black text-white text-sm text-center p-3 rounded-lg shadow-lg">
                            <Image src={assets.qrcode} alt=" " className="w-44" />
                            <p className="p-1">Contribute to Project ❤️</p>
                            <div className={`w-3 h-3 absolute bg-black rotate-45 ${expand ? "right-1/2" : "left-4"} -bottom-1.5`}></div>
                        </div>
                    </div>
                    {expand && <><span>Contribute in Project </span> <Image alt="" src={assets.new_icon} /> </>}
                </div>  
                {/* Profile stuff */}
